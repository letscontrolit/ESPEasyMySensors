...
boolean Plugin_006(byte function, struct EventStruct *event, String& string)
{
  ...
  switch (function)
  {
    ...
    case PLUGIN_INIT:
      {
        if (!msgBaro006)
          msgBaro006 = new MyMessage(event->BaseVarIndex,  V_PRESSURE);
        if (!msgTemp006)
          msgTemp006 = new MyMessage(event->BaseVarIndex + 1, V_TEMP);
        present(event->BaseVarIndex, S_BARO);
        present(event->BaseVarIndex + 1, S_TEMP);
        Serial.print("Present BMP: ");
    ...
    case PLUGIN_READ:
      {
        ...
        send(msgBaro006->set(UserVar[event->BaseVarIndex + 1], 1));
